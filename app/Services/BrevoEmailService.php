...
class BrevoEmailService
{
    ...
    public function sendProfilePicViolationEmail(string $toEmail, string $toName, int $strikeCount, bool $autoBanned = false): bool
    {
        return $this->dispatch(
            toEmail: $toEmail,
            toName: $toName,
            subject: 'Profile picture violation — '.config('app.name'),
            htmlView: 'mail.profile-pic-violation',
            textView: 'mail.profile-pic-violation-plain',
            data: [
                'name' => $this->safeName($toName),
                'strikeCount' => $strikeCount,
                'autoBanned' => $autoBanned,
                'appName' => (string) config('app.name'),
            ],
            type: 'profile-pic-violation',
        );
    }

    public function sendCustomerBannedEmail(string $toEmail, string $toName): bool
    {
        return $this->dispatch(
            toEmail: $toEmail,
            toName: $toName,
            subject: 'Account suspended — '.config('app.name'),
            htmlView: 'mail.customer-banned',
            textView: 'mail.customer-banned-plain',
            data: [
                'name' => $this->safeName($toName),
                'appName' => (string) config('app.name'),
            ],
